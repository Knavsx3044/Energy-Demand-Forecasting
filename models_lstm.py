...
def train_lstm(
    ...
    # ── NEW: Support for stacked LSTM layers (n_lstm_layers: 1 or 2) ──────
    # Default is 1 for full backward compatibility with existing yaml configs.
    n_lstm_layers = int(cfg.get("n_lstm_layers", 1))
    # Second layer units = half of first layer, minimum 16
    units_layer2 = max(units // 2, 16)
    # ───────────────────────────────────────────────────────────────────────

    if n_lstm_layers >= 2:
        # Two-layer stacked LSTM:
        # Layer 1: return_sequences=True to pass full sequence to Layer 2
        # Layer 2: return_sequences=False to output single context vector
        head_layers = [
            LSTM(units, input_shape=(n_steps, X_seq.shape[2]), return_sequences=True),
            Dropout(dropout),
            LSTM(units_layer2, return_sequences=False),
            Dropout(dropout),
        ]
    else:
        # Original single-layer LSTM (default, backward compatible)
        head_layers = [
